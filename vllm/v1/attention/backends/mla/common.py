    ...
    for local_chunk_seq_len, local_context_lens in zip(
        ...
        for rank, local_context_len in enumerate(local_context_lens):
            if local_context_len != 0:
                ...
                context_len_across_rank += local_context_len_sum[rank]
...
class MLACommonImpl(MLACommonBaseImpl[M], Generic[M]):
    ...
    def _context_parallel_compute_prefill_context(
        ...
        for i in range(iters):
            ...
            local_context_len_sum = np.sum(local_context_lens_allrank, axis=0).tolist()
            assert toks * dcp_world_size <= cur_allgather_workspace.shape[0]
            cur_allgather_kvcache = cur_allgather_workspace[: sum(local_context_len_sum)]
            
            cur_allgather_kvcache.copy_(
                get_dcp_group().all_gatherv(local_gathered_kvcache, dim=0, sizes=local_context_len_sum)
            )
            ...
            kv_c_normed, k_pe = reorg_kvcache(
                ...
                local_context_len_sum=local_context_len_sum,
            )
